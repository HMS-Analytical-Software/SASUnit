...
data work.actual;
   set work._input;
   %_sasunit_render_assertReportAct (i_sourceColumn=tst_act, i_expectedColumn=tst_exp,o_html=1,o_targetColumn=_output);
run;
...
data work.expected;
   ...
   _output  = catt ("^{style [flyover=""", i_linktitle, """ url=""", i_linkColumn, """]", " " !! hlp, "}");
run;
%initTestcase(i_object=_sasunit_render_assertReportAct.sas, i_desc=Both result are present - test_res eq 2);
data work.actual;
   set work._input;
   %_sasunit_render_assertReportAct (i_sourceColumn=tst_act, i_expectedColumn=tst_exp,o_html=1,o_targetColumn=_output);
run;
...
data work.expected;
   ...
   _output  = catt ("^{style [flyover=""", i_linktitle, """ url=""", i_linkColumn, """]", " " !! hlp, "}");
run;
%initTestcase(i_object=_sasunit_render_assertReportAct.sas, i_desc=Both result are present - test_res ne 2);
data work.actual;
   set work._input;
   %_sasunit_render_assertReportAct (i_sourceColumn=tst_act, i_expectedColumn=tst_exp,o_html=1,o_targetColumn=_output);
run;
