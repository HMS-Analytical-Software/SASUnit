/** \file
   ...
   \brief      Test of _render_assertManualExp.sas
...
data work.expected;
   ...
   _output  = "";
run;
%initTestcase(i_object=_render_assertManualExp.sas, i_desc=%str(Target column %(expected%) will always be empty));
data work.actual;
   set work._input;
   %_render_assertManualExp (i_sourceColumn=Text,o_html=1,o_targetColumn=_output);
run;
