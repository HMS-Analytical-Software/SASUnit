/** \file
   ...
   \brief      Test of _render_assertLogMsgAct.sas
...
data work.expected;
   ...
   _output  = hlp;
run;
%initTestcase(i_object=_render_assertLogMsgAct.sas, i_desc=Sourcecolumn contains missing value);
data work.actual;
   set work._input;
   %_render_assertLogMsgAct (i_sourceColumn=Text,o_html=1,o_targetColumn=_output);
run;
...
data work.expected;
   ...
   _output  = hlp;
run;
%initTestcase(i_object=_render_assertLogMsgAct.sas, i_desc=Sourcecolumn contains non-missing value);
data work.actual;
   set work._input;
   %_render_assertLogMsgAct (i_sourceColumn=Text,o_html=1,o_targetColumn=_output);
run;
