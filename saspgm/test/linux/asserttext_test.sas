...
%assertText(i_script            =&g_sasunit_os./NotExistendFile.sh
...
/* test case 2 ------------------------------------ */
   ...
   %assertText(i_script            =&g_sasunit_os./assertText_diff.sh
...
%initTestcase(
             i_object   = asserttext.sas
            ,i_desc     = Specific tests with modifiers for linux compare tool 'diff'
   )
%endTestcall()
...
/* test case 7 ------------------------------------ */
   %assertText(i_script            =&g_sasunit_os./assertText_diff.sh
              ...
              ,i_expected_shell_rc =0
              ,i_modifier          =-i
              ,i_desc              =%str(Files do not match, one letter in different case, but -i modifier used -> Comparision is OK)
              );
...
/* test case 9 ------------------------------------ */
   %assertText(i_script            =&g_sasunit_os./assertText_diff.sh
              ...
              ,i_expected_shell_rc =0
              ,i_modifier          =-b
              ,i_desc              =%str(Files do match, extra blanks in text, but compress blanks modifier -b used)
              );
