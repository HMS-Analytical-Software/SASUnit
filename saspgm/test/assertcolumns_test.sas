...
%let g_DokumentFileExistsNoXCMD=%eval(1-(%sysfunc(getoption(XCMD)) = NOXCMD));
...
%assertEquals(i_expected=&g_DokumentFileExistsNoXCMD., i_actual=%sysfunc(exist(_acLib._columns_exp)), i_desc=i_expected in testout)
