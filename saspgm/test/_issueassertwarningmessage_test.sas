...
%macro testcase(i_object=_issueassertwarningmessage.sas, i_desc=%str(Call with logging level Info, which is default));
   /*****************
   documentation
   ******************
   setup  [...] 
...
%macro testcase(i_object=_issueassertwarningmessage.sas, i_desc=%str(Call with logging level Info, and debug message));
   /*****************
   documentation
   ******************
   setup  [...] 
   ...
   *****************/
   ...
   %assertLogMsg  (i_logMsg=^WARNING: Dies ist meine Warnmeldung!, i_not=1);
