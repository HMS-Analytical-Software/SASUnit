   ...
   *** Call all associated submacros for rendering specific assert reports ***;
   %do l_NumAssert=1 %to &_numAsserts;
      ...
      %if (%length(&l_NumAssertSubstr.) > 13) %then %do;
         %let l_NumAssertSubstr = %substr(&l_NumAssertSubstr.,1,13);
      %end;
      %if (%sysfunc (fileexist(&g_sasunit./_sasunit_render_&l_NumAssertSubstr.rep.sas))) %then %do;
         proc sql noprint;
         ...
         %do l_TestAssert=1 %to &l_NumTests.;
            ...
            %_sasunit_render_&l_NumAssertSubstr.rep(i_assertype=&&asserttype&l_NumAssert.,i_repdata=&i_repdata.,i_scnid=&i_scnid.,i_casid=&i_casid.,i_tstid=&l_cTestAssert.,o_html=&o_html.,o_path=&o_path.);
         %end;
      %end;
   %end;
