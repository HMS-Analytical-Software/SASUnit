/** 
   ...
   \brief      Issues an info message to a logger
...
*//** \cond */
%macro _issueInfoMessage(loggername, message);
   ...
   %if (%length(&message.)=0) %then %do; 
      ...
      %return;
   %end;
   %let _rc = %sysfunc(log4sas_logevent(&loggername., Info, &message.));
   %if (&_rc ne 0) %then %do;
      %put ERROR: _rc is NOT null: &_rc.;  
   %end;
%mend _issueInfoMessage;
/**  \endcond */