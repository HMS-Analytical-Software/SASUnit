...
%IF &l_msg_found %THEN %DO;
   %LET l_actual = Meldung gefunden;
%END;
%ELSE %DO;
   %LET l_actual = Meldung nicht gefunden;
%END;

%LOCAL l_expected l_assert_failed;
%IF &i_not %THEN %DO;
   %LET l_expected = Meldung "&i_logmsg" nicht vorhanden;
   %LET l_assert_failed = &l_msg_found;
%END;
%ELSE %DO;
   %LET l_expected = Meldung "&i_logmsg" vorhanden;
