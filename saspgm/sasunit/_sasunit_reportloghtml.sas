/** \file
   ...
   \sa         \$HeadURL: file:///P:/hms/00507_sasunit/svn/branches/v00.903L/saspgm/sasunit/_sasunit_reportloghtml.sas $
...
%LOCAL l_error_count l_warning_count l_curError l_curWarning;
DATA _null_;
   INFILE "&l_log" END=eof TRUNCOVER;
   INPUT logline $char255.;
   IF index (logline, "&g_error:")   = 1 THEN error_count+1;
   IF index (logline, "&g_warning:") = 1 THEN warning_count+1;
   IF eof THEN DO;
...
DATA _null_;
   ...
   IF index (logline, "&g_error:")   = 1 THEN DO;
      error_count+1;
      ...
      PUT '</span>';
   END;
   ELSE IF index (logline, "&g_warning:") = 1 THEN DO;
      warning_count+1;
