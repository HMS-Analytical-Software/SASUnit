...
%macro _sasunit_getTestSubfolder (i_assertType=
   ...
   %if %nrbquote (&i_assertType.) = %str() %then %do;
      %put &g_error: Please specify a value for i_assertType.;
      %RETURN;
   %end;
   %if %nrbquote (&i_root.) = %str() %then %do;
      %put &g_error: Please specify a value for i_root.;
      %RETURN;
   %end;
   %if not %_sasunit_existdir (&i_root.) %then %do;
      %put &g_error: Please specify a valid directory for i_root.;
      %RETURN;
   %end;
   ...
   %if %nrbquote (&i_casid.) = %str() %then %do;
      %put &g_error: Please specify a value for i_casid.;
      %RETURN;
   %end;
