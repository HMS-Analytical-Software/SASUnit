/**
   \file
   \ingroup    SASUNIT_TESTPROJECT 

   \brief      No source code - only file grouping for program documentation
   
   \details    This file does not contain any source code. Its purpose is to define groups to be used in SASUnit's program documentation. The group structure is being
               defined by defgroup and ingroup instructions above. Defined groups may be used with an ingroup instruction in other source code files. 

   @version    \$Revision: 470 $
   \author     \$Author: klandwich $
   \date       \$Date: 2016-08-22 11:52:57 +0200 (Mo, 22 Aug 2016) $

   \sa         For further information please refer to https://sourceforge.net/p/sasunit/wiki/User%27s%20Guide/
               Here you can find the SASUnit documentation, release notes and license information.
   @sa         \$HeadURL: https://svn.code.sf.net/p/sasunit/code/trunk/example/saspgm/pgmdoc_groups.sas $
   @copyright  This file is part of SASUnit, the Unit testing framework for SAS(R) programs.
               For copyright information and terms of usage under the GPL license see included file readme.txt
               or https://sourceforge.net/p/sasunit/wiki/readme/.

   \defgroup   SASUNIT        SASUnit automated testing
   \brief      SASUnit testing project program documentation

   \defgroup   SASUNIT_TESTPROJECT   SASUnit Test Project
   \brief      SASUnit project for testing

   \defgroup   SASUNIT_TP_PGM   Units under test
   \brief      SAS macros to be tested in the testing project
   \ingroup    SASUNIT_TESTPROJECT

   \defgroup   SASUNIT_TP_TEST  Test Scenarios
   \brief      Tests for the SAS macros in the testing project
   \ingroup    SASUNIT_TESTPROJECT

   \defgroup   SASUNIT_CNTL   SASUnit macros for control of the test suite
   \brief      Initialize SASUnit, run tests and generate reports
   \ingroup    SASUNIT

   \defgroup   SASUNIT_SCN    SASUnit macros for setup of test scenarios
   \brief      Control the collection of test results
   \ingroup    SASUNIT

   \defgroup   SASUNIT_ASSERT SASUnit macros for assertions
   \brief      Assertions compare actual outcomes to expectations
   \ingroup    SASUNIT
   
   \defgroup   SASUNIT_UTIL   Utility Macros
   \brief      Contains utility macros
   \ingroup    SASUNIT

   \defgroup   SASUNIT_UTIL_OS_WIN   Utility Macros specific for Windows OS
   \brief      Contains utility macros specific for Windows os
   \ingroup    SASUNIT_UTIL

   \defgroup   SASUNIT_UTIL_OS_LINUX   Utility Macros specific for Linux OS
   \brief      Contains utility macros specific for Linux os
   \ingroup    SASUNIT_UTIL

   \defgroup   SASUNIT_UTIL_OS_UNIX_AIX  Utility Macros specific for Unix AIX OS
   \brief      Contains utility macros specific for Unix AIX os
   \ingroup    SASUNIT_UTIL

   \defgroup   SASUNIT_REPORT Reporting 
   \brief      Contains macros for reporting
   \ingroup    SASUNIT_UTIL
*/
